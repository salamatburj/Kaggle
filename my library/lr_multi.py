...
@author: e0046971
"""
''' I will start by writing code for multi classifier. We need to change y to matrix of
n_class,m n_class number of classes, m - number of examples

''' 
import numpy as np
Y=np.array([[1, 2, 3, 1, 2, 2, 2, 3]])
unique=np.unique(Y)
unique=unique.reshape(unique.shape[0],1) # changing shape n_class,1
Ym=np.dot(np.ones((unique.shape[0],1)),Y) # matrix with same values in row 
Ybool=(Ym==unique).astype(int) # integer bool matrix n_class,m
